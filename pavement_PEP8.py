...
if sys.platform == "darwin":
    WINDOWS_PYTHON = {
        "3.4": ["wine", os.environ['HOME'] +
                "/.wine/drive_c/Python34/python.exe"],
        "3.3": ["wine", os.environ['HOME'] +
                "/.wine/drive_c/Python33/python.exe"],
        "3.2": ["wine", os.environ['HOME'] +
                "/.wine/drive_c/Python32/python.exe"],
        "2.7": ["wine", os.environ['HOME'] +
                "/.wine/drive_c/Python27/python.exe"],
        "2.6": ["wine", os.environ['HOME'] +
                "/.wine/drive_c/Python26/python.exe"],
    }
...
def _build_mpkg(pyver):
    ...
    if os.environ.get('MACOSX_DEPLOYMENT_TARGET', None) == "10.6":
        ldflags = "-undefined dynamic_lookup" \
                  " -bundle -arch i386 -arch x86_64 -Wl,-search_paths_first"
    else:
        ldflags = "-undefined dynamic_lookup " \
                  "-bundle -arch i386 -arch ppc -Wl,-search_paths_first"

    ldflags += " -L%s" % os.path.join(os.path.dirname(__file__), "build")
    sh_command = "LDFLAGS='%s' %s setupegg.py bdist_mpkg"
    sh(sh_command % (ldflags, " ".join(MPKG_PYTHON[pyver])))
...
def dmg(options):
    ...
    mpkg_target = os.path.join(content,
                               "numpy-%s-py%s.mpkg" % (FULLVERSION, pyver))
